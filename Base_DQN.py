    ...
    while epoch <= args.nb_epochs:
        ...
        while not dead:
            ...
            if epoch > args.replay_start_size and epoch % args.update_frequency == 0:
                ...
                # 计算平均q值
                # q_values = q_network(torch.Tensor(obs).unsqueeze(0).to(args.device))
                #
                # epoch_qs.append(q_values.mean())

                # 计算损失函数L
                current_q_value = q_network(data.observations).gather(
                    1, data.actions).squeeze()
                loss = F.huber_loss(y, current_q_value)  # 返回值为torch.tensor类型(batch_size, num_actions)
                ...
                #     writer.add_scalar("Loss/Average Loss", avg_loss, epoch)
                #     epoch_losses = []
                #
                #     # 计算平均Q value
                #     ave_q_value = sum(epoch_qs) / len(epoch_qs)
                #     writer.add_scalar("average q", ave_q_value, epoch)
                #     epoch_qs = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nb_epochs", type=int,
                        default=10_000_000, help='训练轮数')  # 下次改成10_000_000
