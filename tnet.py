...
class DQN(nn.Module):
    def __init__(self, nb_actions):
        ...
        # 实例化一个神经网络作为primary net
        self.primary_net = DQNNet(nb_actions)

        # 实例化一个神经网络作为target net
        self.target_net = DQNNet(nb_actions)

    def update_target_net(self):
        # 更新target network的参数
        self.update_target_net.load_state_dict(self.state_dict())
...
def Deep_Q_Learning(env, args):
    ...
    # 使用随机权重初始化q_network, target_network
    primary_network = DQN(env.action_space.n).to(args.device)
    target_network = DQN(env.action_space.n).to(args.device)

    # 使用随机权重初始化primary_net
    optimizer = torch.optim.Adam(primary_network.parameters(), lr=args.lr)

    # 初始化target net参数，让他与primary net参数相匹配
    ...
    while epoch <= args.nb_epochs:
        ...
        while not dead:
            ...
            if random.random() < epsilon:
                ...
                q_values = primary_network(torch.Tensor(
            ...
            if epoch > args.replay_start_size and epoch % args.update_frequency == 0:
                ...
                # 从D中随机采样一个批次的转移数据
                data = rb.sample(args.batch_size)  # 默认设置为32
                with torch.no_grad():
                    # 计算目标值y
                    max_q_value, _ = target_network(
                        data.next_observations).max(dim=1)
                    y = data.rewards.flatten() + args.discount_factor * max_q_value * \
                        (1 - data.dones.flatten())

                # 计算损失函数L
                current_q_value = primary_network(data.observations).gather(
                ...
                # 在每个 epoch 结束后计算下一个状态的平均 Q 值
                with torch.no_grad():
                    next_q_values = target_network(data.next_observations)
                    epoch_qs.append(next_q_values)

            epoch += 1

            # 每隔一定轮数，计算平滑后的Q值、loss值、以及rewards值绘制图像，并保存模型
            if (epoch % 50_000 == 0) and epoch > 0:
                if len(epoch_qs) > 0:
                    avg_q_value = torch.mean(torch.stack(epoch_qs))
                else:
                    avg_q_value = None
                if avg_q_value is not None:
                    smoothed_q_value.append(avg_q_value.item())
                    epoch_qs = []
                ...
                if avg_loss is not None:
                    smoothed_losses.append(avg_loss.item())
                    epoch_losses = []
                ...
                if not os.path.exists('Models'):
                    os.mkdir('Models')
                torch.save(primary_network.state_dict(), 'Models/' + args.model_name + '.pth')

            progress_bar.update(1)

        # 每隔一定轮数，更新一次target-net
        if (epoch % args.update_period == 0) and epoch > 0:
            target_network.load_state_dict(primary_network.state_dict())

        rewards.append(total_rewards)

    for i, smoothed_reward in enumerate(smoothed_rewards):
        # 将平均奖励值写入 TensorBoard 日志
        writer.add_scalar('Average Reward', smoothed_reward, i)

    for i, smoothed_losses in enumerate(smoothed_losses):
        # 将平均奖励值写入 TensorBoard 日志
        writer.add_scalar('Average Loss', smoothed_losses, i)

    for i, smoothed_q_value in enumerate(smoothed_q_value):
        # 将平均奖励值写入 TensorBoard 日志
        writer.add_scalar('Average Q Value', smoothed_q_value, i)
...
if __name__ == "__main__":
    ...
    #                     default=0.001, help='平滑更新target Q network的参数')
    parser.add_argument("--update_period", type=int,
                        default=600, help='平滑更新target Q network的参数')
